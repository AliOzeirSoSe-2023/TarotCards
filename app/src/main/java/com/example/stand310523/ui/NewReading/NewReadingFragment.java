...
import com.example.stand310523.databinding.FragmentNewreadingBinding;
import java.util.Random;
public class NewReadingFragment extends Fragment {

    private FragmentNewreadingBinding binding;
    ...
    private final String[] imageTexts = new String[]{
    ...
    private boolean[] isCardRevealed = {false, false, false};
    private int k=0;
    public View onCreateView(@NonNull LayoutInflater inflater,
                             ViewGroup container, Bundle savedInstanceState) {
        binding = FragmentNewreadingBinding.inflate(inflater, container, false);
        View root = binding.getRoot();

        Button btnChooseReading = binding.btnChooseReading;
        ImageView[] tarotImages = {binding.tarotImage1, binding.tarotImage2, binding.tarotImage3};
        TextView[] imageDescriptions = {binding.imageDescription1, binding.imageDescription2, binding.imageDescription3};

        btnChooseReading.setOnClickListener(view -> {
            String[] choices = {"Single card Reading", "Three card Reading"};
            AlertDialog.Builder builder = new AlertDialog.Builder(getContext());
            builder.setTitle("Choose Your Reading")
                    .setItems(choices, (dialog, which) -> {

                        for (ImageView tarotImage : tarotImages) {
                            tarotImage.setOnClickListener(null);
                        }
                        if (which == 0) { // Single card Reading
                            k = 1;
                            isCardRevealed[0] = false;
                            imageDescriptions[0].setVisibility(View.GONE);
                            int imageResource = getResources().getIdentifier("cardbackside", "drawable", getContext().getPackageName());
                            tarotImages[0].setImageResource(imageResource);
                            for (int i = 1; i < 3; i++) {
                                imageDescriptions[i].setVisibility(View.GONE);
                                tarotImages[i].setVisibility(View.INVISIBLE);
                                isCardRevealed[i] = true;

                            }
                        } else if (which == 1) { // Three card Reading
                            k = 3;
                            for (int i = 0; i < 3; i++) {
                                isCardRevealed[i] = false;
                                imageDescriptions[i].setVisibility(View.GONE);
                                int imageResource = getResources().getIdentifier("cardbackside", "drawable", getContext().getPackageName());
                                tarotImages[i].setImageResource(imageResource);
                                tarotImages[i].setVisibility(View.VISIBLE);
                            }
                        }
                        for (int i = 0; i < k; i++) {
                            int finalI = i;
                            tarotImages[i].setOnClickListener(view1 -> {
                                if (!isCardRevealed[finalI]) {
                                    int randomIndex = new Random().nextInt(imageResources.length);
                                    int imageResource = getResources().getIdentifier(imageResources[randomIndex], "drawable", getContext().getPackageName());
                                    tarotImages[finalI].setImageResource(imageResource);
                                    imageDescriptions[finalI].setText(imageTexts[randomIndex]);
                                    imageDescriptions[finalI].setVisibility(View.VISIBLE);
                                    isCardRevealed[finalI] = true;
                                } else {
                                    AlertDialog.Builder builder1 = new AlertDialog.Builder(getContext());
                                    View dialogView = getLayoutInflater().inflate(R.layout.fragment_newreading, null);

                                    ImageView dialogImage = dialogView.findViewById(R.id.dialogImage);
                                    TextView dialogText = dialogView.findViewById(R.id.dialogText);

                                    int imageResource = getResources().getIdentifier(imageResources[finalI], "drawable", getContext().getPackageName());
                                    dialogImage.setImageResource(imageResource);
                                    dialogText.setText(imageTexts[finalI]);
        ...
        Button saveYourReadingButton = binding.saveYourReadingButton;
        DBHelperSaveAnswer dbHelper = new DBHelperSaveAnswer(getContext());

        saveYourReadingButton.setOnClickListener(view -> {
            if(isCardRevealed[0] && isCardRevealed[1] && isCardRevealed[2]){
                AlertDialog.Builder builder = new AlertDialog.Builder(getContext());
                View dialogView = getLayoutInflater().inflate(R.layout.save_reading_dialog, null);

                EditText etMindState = dialogView.findViewById(R.id.etMindState);
                EditText etCardsInterpretation = dialogView.findViewById(R.id.etCardsInterpretation);
                ...
                btnSave.setOnClickListener(v -> {
                    String mindState = etMindState.getText().toString();
                    String cardsInterpretation = etCardsInterpretation.getText().toString();
                    String spiritualName = spiritualNametosend;

                    dbHelper.saveReading( spiritualName, mindState, cardsInterpretation);
                ...
                Toast.makeText(getContext(), "Reveal all cards first", Toast.LENGTH_SHORT).show();
            }
        });
